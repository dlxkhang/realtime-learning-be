...
class Middleware {
    async isVerify(req: UserInfoRequest, res: Response, next: NextFunction) {
        try {
            ...
            if (!user) {
                return res.status(USER_ERROR_CODE.EMAIL_NOT_FOUND.statusCode).send(USER_ERROR_CODE.EMAIL_NOT_FOUND.message)
            }
            if (!user.isVerified) {
                return res.status(AUTH_ERROR_CODE.INVALID_EMAIL_TOKEN.statusCode).send(AUTH_ERROR_CODE.INVALID_EMAIL_TOKEN.message)
            }
            next()
        } catch (err) {
